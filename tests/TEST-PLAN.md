...
* upgrade de schéma du marker requis (version drift) mais échoué => boot/update refusé (erreur explicite)
* changement de schéma `/.retaia` sans incrément de `version` => non conforme (gate bloquant)
